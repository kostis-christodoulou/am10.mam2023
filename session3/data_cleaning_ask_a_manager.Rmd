...
> Exercise 4: `countrycode::countrycode()` to fix different spellings of countries

One of the most useful packages is `countrycode` as it can fix different speelings for countries. In our dataset, there was no drop-down menu to choose one's country in the survey,so respondents have written, e.g., United States, USA, U.S.A., US, etc. 

```{r countrycode}

ask_a_manager <- ask_a_manager %>%
  mutate(clean_country = 
           countrycode(country,
                       origin = 'country.name',
                       destination = 'country.name'
                       ))

# check how many clean_country entries you now have-- 
# waht about the NAs?
ask_a_manager %>% 
  count(clean_country, sort=TRUE)
